...
class _DummyAnimaTransformer(torch.nn.Module):
    def __init__(self, hidden_size: int = 8):
        ...
        self.adapter = torch.nn.Linear(hidden_size, hidden_size, bias=False)
    ...
    def preprocess_text_embeds(self, qwen_hidden, t5_ids):
        ...
        seq = t5_ids.shape[1]
        source = qwen_hidden[:, :seq, :]
        if source.shape[1] < seq:
            source = torch.nn.functional.pad(source, (0, 0, 0, seq - source.shape[1]))
        return self.adapter(source).to(dtype=qwen_hidden.dtype, device=qwen_hidden.device)

    def forward(self, noisy_latents, t_values, cross, padding_mask=None):
        del t_values, padding_mask
        cross_term = cross.mean(dim=(1, 2)).view(-1, 1, 1, 1, 1).to(dtype=noisy_latents.dtype, device=noisy_latents.device)
        return noisy_latents + cross_term
...
def test_process_batch_raises_if_t5_ids_missing():
    native_entry = import_native_entry()
    trainer = native_entry.AnimaNetworkTrainer()
    accelerator = _DummyAccelerator()
    qwen_model = _DummyQwenModel()
    ...
    batch = {
        ...
        "input_ids_list": [
            torch.randint(0, 100, (1, 6), dtype=torch.long),
    ...
    with pytest.raises(ValueError, match="T5 ids are required"):
...
def test_process_batch_grad_flows_through_preprocess_text_embeds():
    native_entry = import_native_entry()
    trainer = native_entry.AnimaNetworkTrainer()
    accelerator = _DummyAccelerator()
    qwen_model = _DummyQwenModel()
